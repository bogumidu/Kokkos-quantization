...
void algorithm::quantizationAlgorithmKokkos(Face *faces, int accuracy, int index, std::list<Voxel> *voxelStore) {
    Face face = faces[index];
    auto vertices = face.getVertices();
    auto textures = face.getTextures();
    // exit state, voxel creation
    if (accuracy <= 0 || (utils::almostEqual(vertices[0], vertices[1]) && utils::almostEqual(vertices[1], vertices[2]))) {
        ...
        Voxel voxel2((int) vertices[1].front(), (int) vertices[1].at(1), (int) vertices[1].back(), color);
        Voxel voxel3((int) vertices[2].front(), (int) vertices[2].at(1), (int) vertices[2].back(), color);
        voxelStore->push_back(voxel1);
        voxelStore->push_back(voxel2);
        voxelStore->push_back(voxel3);
        return;
    }
    // tessellation and recursion
    accuracy--;
    ...
    // TODO: not sure if this should create 4 functor instances or do it in a loop
    for (int i = 0; i < 4; i++) {
        quantizationAlgorithmKokkos(new_faces, accuracy, i, voxelStore);
    }
}
